...
abstract class Message extends HttpMessage {
  ...
  def cacheControl_=(maxAge: Duration) {
    cacheControl = "max-age=" + maxAge.toSeconds.toString + ", must-revalidate"
  }

  /** Get charset from Content-Type header */
  def charset: Option[String] = {
    contentType.foreach { contentType =>
      val parts = contentType.split("\\s*;\\s*")
  ...
  def mediaType_=(value: String) {
    contentType match {
      case Some(contentType) =>
        val parts = contentType.split("\\s*;\\s*").take(2)
        if (parts.length == 2) {
  ...
  def contentString: String = getContent.toString(Utf8)
  ...
  def withWriter[T](f: Writer => T): T = {
    withOutputStream { outputStream =>
      val writer = new OutputStreamWriter(outputStream, Utf8)
      val result = f(writer)
      writer.close()
...
object Message {
  ...
  def httpDateFormat(date: Date): String = new DateTime(date.getTime).toString(HttpDateFormat)
}
