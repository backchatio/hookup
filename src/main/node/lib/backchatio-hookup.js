 ...
 * 
 * @event 
 * @param {String} name The name of the event, possible values: open, reconnecting, ack_failed, data, error, disconnect.
 ...
 * @param {String} options.uri The uri to connect to.
 * @param {Object} [options.reconnectSchedule = { min: 1, min: 300 }]  
 *                 The schedule (an object with a min and max property) to use for backing-off.
 * @param {WireFormat} [options.wireFormat = new WireFormat] The wire format to use with this client.
 * @param {FileBuffer} [options.buffer = undefined] The buffer to use if you want this client to fallback to buffering. 
 * @property {Object} reconnectSchedule The schedule (an object with a min and max property) to use for backing-off when reconnecting.
 * @property {String} uri The uri this client connects to.
 */
...
util.inherits(HookupClient, events.EventEmitter);

/**
 * The default reconnection schedule
 * This is an object with 2 properties, min and max.
 * The values entered are seconds
 *
...
_.extend(HookupClient.prototype, /** @lends HookupClient.prototype */ {
   ...
   *
   * @param {String|Object} message The message to send
   * @param {Object} [options.timeout = 5000] The timeout for the ack in milliseconds
   */
